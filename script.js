...
[...buttons].forEach((button)=>
  button.addEventListener('click',(ev)=>
  {
    ...
    if(action === "toggle") {
      ...
      if(!calculatorStatus) //se sterge istoricul la inchidere
      {
        historyEl.innerHTML="";
      }
   ...
   // am adaugat disabled pe butoane 
    buttons.forEach((btn) => {
      if (btn !== buttonStart) { // nu se modifica si butonul I/O
...
//Daca se apasa o cifra (0-9)
   ...
   // verifica daca a fost apasat un operator
      if(action==="operation") {
        ...
        const currentValueParse = Number(currentValue);//converteste currentValue din text in numar
        const secondaryValueParse = Number(secondaryValue); // converteste secondaryValue din text in numar 

        console.log(`First value: ${currentValue}, Second value: ${secondaryValue}`);
      } 
...
{ //trebuie verificat daca valoarea curenta,  valoarea secundara si opearatia nu sunt nule  
  if(currentValue !== "" && secondaryValue !== "" && operation !== "") //daca nu sunt nule 
  { 
